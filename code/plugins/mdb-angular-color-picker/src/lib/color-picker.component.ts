  ...
  private _initDrag(): void {
    this._ngZone.runOutsideAngular(() => {
      ...
      const pointerEnd$ = merge(
        fromEvent<MouseEvent>(this._document.body, 'mouseup'),
        fromEvent<TouchEvent>(this._document.body, 'touchend')
      );

      const pointerDrag$ = merge(
        fromEvent<MouseEvent>(this._document.body, 'mousemove'),
        fromEvent<TouchEvent>(this._document.body, 'touchmove')
      );
